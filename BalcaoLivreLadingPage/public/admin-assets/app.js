...
function setView(view) {
  ...
  if (view === "visits") {
    ...
    window.setTimeout(() => {
      initVisitsMap();
      visitsMapInstance?.invalidateSize();
    }, 80);
  }
}
...
function renderVisitsNearby() {
  ...
  if (!visible.length) {
    list.innerHTML = '<div class="visits-empty">Ative a localização para buscar estabelecimentos reais por perto.</div>';
  }
  qs("#showAllNearbyVisits").textContent = visible.length
    ? (state.showAllNearbyVisits ? "Mostrar menos" : "Ver todas")
    : "Nenhum local carregado";
  qs("#showAllNearbyVisits").disabled = !visible.length;
}
...
async function setSupportStatus(id, status) {
  ...
  if (isLocalPreview) {
    ...
    if (status === "EM_ATENDIMENTO") {
      ...
      showSupportNotice("Atendimento assumido por Isabel Costa.");

...
class AgendaAccountError extends Error {
...
async function authenticateSupabaseUser(request: Request): Promise<AuthenticatedUser> {
...
async function ensureAccount(user: AuthenticatedUser) {
  const now = Date.now();
  await getAgendaD1()
    .prepare(
...
function accountState(row: AccountRow, now = Date.now()): AccountState {
...
export async function putAgendaAccountState(request: Request) {
  ...
  return jsonResponse({ ok: true, ...accountState(updated) });
}

...
function storeScheduleText(menu = currentMenu) {
  if (!menu || menu.schedule_enabled === false) return "Atendimento da loja";
  const open = String(menu.open_time || "00:00").trim() || "00:00";
  const close = String(menu.close_time || "00:00").trim() || "00:00";
  return open === close ? "Aberto 24 horas" : `Horario ${open} as ${close}`;
}
...
function renderMenu(menu, items) {
  ...
  if (openText) {
    openText.innerHTML = `${iconSvg("store", "status-icon")}<b class="dot"></b> ${isOpen ? "Aberto agora" : "Fechado pelo horario"}`;
    openText.classList.toggle("closed", !isOpen);
  }
  const waitText = qs("#waitTimeText");
  if (waitText) waitText.textContent = storeScheduleText(menu);
  renderActions(menu);
